...
class PixelPicker:
    def __init__(self, figure, rect_colls, pick_button, erase_button, interpolation):
        self.xys = set()
    ...
    def get_interpolated_xy(self, xy, previous_xy):
        ...
        xy = xy if is_not_inverted == True else (xy[1], xy[0])
        previous_xy = previous_xy if is_not_inverted == True else (previous_xy[1], previous_xy[0])
        ...
        for x in range (previous_xy[0], xy[0], -1 if previous_xy[0] > xy[0] else 1):
            ...
            interpolated_xy.append((int(round(x if is_not_inverted == True else y)), int(round(y if is_not_inverted == True else x))))
...
def pick_pixels(class_num=0, color=(1, 0, 0, 0.7), pick_button=1, erase_button=3, interpolation=True):
    """
    ...
    :paramn interpolation: True = Interpolates points in between when moving mouse too fast, False = No interpolation
    ...
    picker = PixelPicker(fig, rect_colls, pick_button, erase_button, interpolation)
